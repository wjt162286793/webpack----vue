<template>
  <div class="modeBox">
    <div class="searchBox">
      <div class="searchItem">
        <span class="searchLabel">资产名称</span
        ><el-input
          @change="reqList"
          clearable
      ...
      <div class="searchItem">
        <span class="searchLabel">资产编号</span
        ><el-input
          @change="reqList"
          clearable
          class="serachFormItem"
      ...
      <div class="searchItem">
        ...
        >
          <el-option
            v-for="item in riskWordlist.riskLevelOptions"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          />
      ...
      <div class="searchItem">
        ...
        <el-button type="primary" @click="downLoad" v-show="selectionDoneFlag"
          >下载</el-button
        >
      </div>
    </div>
    <div class="tableBox">
      ...
      >
        ...
        <el-table-column label="操作" fixed="right" width="180">
          <template #default="scope">
            <el-button type="primary" link @click="openRiskInfo(scope.row)"
              >查看</el-button
            >
            <el-button type="primary" link @click="openFlow(scope.row)"
              >流程模型</el-button
            >
          </template>
        </el-table-column>
  ...
  <el-drawer v-model="drawer" :direction="direction" size="75%">
    ...
    <template #default>
      ...
      <div class="rowBox">
        <el-table
          :data="activeRecord.recordList"
          style="width: 100%"
          :border="true"
          :stripe="true"
        >
          <el-table-column prop="name" label="操作名称" width="280" />
          <el-table-column label="操作人员" width="280">
            <template #default="scope">
...
const downLoad = () => {
  ...
  let blob_file = new Blob([content], {
...
const getriskPostion = (info) => {
  return riskWordlist.riskPositionList.find((item) => item.value === info)
    .label;
};
...
const getLevel = (info) => {
  return riskWordlist.riskLevelOptions.find((item) => item.value === info)
    .label;
};
...
const openFlow = (info) => {
  router.push({ path: "/dashboard/risk/riskModel", query: { id: info.uuid } });
};
onMounted(() => {
  reqList();
});
