    ...
    <div class="mainBox">
        <div class="search">
            <div class="searchItem">
                <span class="searchLable">用户名:</span><vxe-input v-model="query.userName" placeholder="输入用户名" size="small"
                    type="search" clearable @clear="reqList" @search-click="reqList"></vxe-input>
            </div>
            <div class="searchItem">
                <span class="searchLable">用户账号:</span> <vxe-input v-model="query.name" placeholder="输入用户账号" size="small"
                    type="search" clearable @clear="reqList" @search-click="reqList"></vxe-input>
            </div>
            <div class="searchItem">
                <span class="searchLable">用户角色:</span> <vxe-select v-model="query.role" placeholder="选择用户角色" size="small"
                    @change="reqList">
                    <vxe-option v-for="(item, index) in roleList" :key="index" :value="item.name"
                        :label="item.cname"></vxe-option>
                </vxe-select>
            </div>
            <div class="searchItem">
                <vxe-button status="primary" content="查询" size="small" @click="reqList"></vxe-button>
            </div>
        </div>
        <div class="tableBox">
        ...
        <vxe-pager background v-model:current-page="pageInfo.currentPage" v-model:page-size="pageInfo.pageSize"
            :total="total"
