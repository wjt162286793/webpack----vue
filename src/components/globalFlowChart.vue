  ...
  <div class="contentBox">
    <div class="leftMenu">
      ...
      <draggable
        @start="nodeMoveStart"
        @end="nodeMoveEnd"
        v-model="props.leftNodeList"
      >
        <div
          v-for="(nodeItem, nodeIndex) in leftNodeOptionsList"
          :key="nodeItem.nodeId"
    ...
    <div
      class="canvasBox"
      id="canvasBox"
      @contextmenu.prevent="openMenuFun($event, 'canvasBox')"
      @click.prevent="canvasBoxClick"
    >
      <div
        v-for="(canvasNodeItem, canvasNodeIndex) in infoList"
        :key="canvasNodeIndex"
        :id="canvasNodeItem.id"
      ...
      >
        ...
        <div class="statusIcon">
          <el-icon color="green" v-if="canvasNodeItem.status === 'success'"
            ><CircleCheckFilled
          /></el-icon>
          <el-icon color="#000" v-else-if="canvasNodeItem.status === 'loading'"
            ><Loading
          /></el-icon>
          <el-icon color="red" v-else-if="canvasNodeItem.status === 'error'"
            ><CircleCloseFilled
          /></el-icon>
        </div>
      </div>
    ...
    <div class="contextMenu" :style="contextStyle" v-show="contextMenuShow">
      <div
        v-for="(menuItem, menuIndex) in contextMenuList"
        @click="menuClickFun(menuItem)"
      >
        {{ menuItem.label }}
      </div>
    </div>
  </div>
  <el-dialog v-model="dialogVisible" :title="dialogTitle" width="20%">
...
const menuClickFun = (item) => {
  ...
  contextMenuShow.value = false;
  doneType.value = globalFun[item.value](
    plumbInstance,
    infoList,
    activeNodeInfo,
    activeLineInfo
...
const getInfoList = (info) => {
  ...
  infoList.value.map(
    (item) => (item = Object.assign(item, globalConfig.readyConfig))
  );
  nextTick(() => {
    renderFun();
