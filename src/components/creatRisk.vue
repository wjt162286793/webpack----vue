<template>
    <el-dialog v-model="riskVisible" title="标记风险" width="700px" :destroy-on-close="true">
        ...
        <div v-show="stepValue === 0" class="content">
            <el-form ref="ruleFormRef0" :model="ruleForm0" :rules="rules0" label-width="160px" class="demo-ruleForm"
                status-icon>
                <el-form-item label="资产名称" prop="name">
                    <el-input v-model="ruleForm0.name" :disabled="true" class="form_item" />
                </el-form-item>
                ...
                <el-form-item label="风险定位" prop="riskPosition">
                    <el-radio-group v-model="ruleForm0.riskPosition">
                        <el-radio v-for="(item, index) in riskOptions.riskPositionList" :label="item.value" :key="index">{{
                            item.label }}</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="风险等级" prop="riskLevel">
                    <el-select v-model="ruleForm0.riskLevel" placeholder="Select" size="large" class="form_item">
                        <el-option v-for="item in riskOptions.riskLevelOptions" :key="item.value" :label="item.label"
                            :value="item.value" />
                    </el-select>
                </el-form-item>
            </el-form>
        ...
        <div v-show="stepValue === 1" class="content">
            <el-form ref="ruleFormRef1" :model="ruleForm1" :rules="rules1" label-width="160px" class="demo-ruleForm"
                status-icon>
                <el-form-item label="监控开关" prop="isControl">
                    <el-switch v-model="ruleForm1.isControl" size="large" active-text="打开" inactive-text="关闭" />
                </el-form-item>
                <el-form-item label="监控粒度" prop="controlSize" v-if="ruleForm1.isControl">
                    <el-radio-group v-model="ruleForm1.controlSize">
                        <el-radio v-for="(item, index) in riskOptions.controlSizeList" :label="item.value" :key="index">{{
                            item.label }}</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="监控时间" prop="times" v-if="ruleForm1.isControl">
                    <el-date-picker v-model="ruleForm1.times" type="daterange" start-placeholder="起始时间"  value-format="YYYY-MM-DD"
        ...
        <div v-show="stepValue === 2" class="content">
            <el-form ref="ruleFormRef2" :model="ruleForm2" :rules="rules2" label-width="160px" class="demo-ruleForm"
                status-icon>
                <el-form-item label="审核人员" prop="auditUser">
                    <el-select v-model="ruleForm2.auditUser" placeholder="请选择审核人员" size="large" class="form_item">
                        <el-option v-for="item in userList" :key="item.value" :label="item.label" :value="item.value" />
                    </el-select>
                </el-form-item>
                <el-form-item label="提交说明" prop="explain">
                    <el-input v-model="ruleForm2.explain" :rows="2" type="textarea" placeholder="请输入提交说明"
                        class="form_item" />
                </el-form-item>
                <el-form-item label="通知方式" prop="times" v-if="ruleForm1.times">
                    <el-checkbox-group v-model="ruleForm2.inform">
                        <el-checkbox v-for="item in informList" :key="item" :label="item" size="large"
                            :disabled="item === 'IAM'" />
                    </el-checkbox-group>
                </el-form-item>
            </el-form>
...
let rules0 = {
    riskAlias: [
        {
            required: true, message: "请输入风险别名", trigger: "blur"
        },
        {
            min: 6, message: "账号至少为6位数", trigger: "blur"
        },
        {
            max: 12, message: "账号最多为12位数", trigger: "blur"
        },
        {
    ...
    riskPosition: [
        {
            required: true, message: "风险定位必须选择", trigger: "change"
        },
    ],
    riskLevel: [
        {
            required: true, message: "风险等级必须选择", trigger: "change"
        },
    ]
}
let rules1 = {
    controlSize: [
        {
            required: true, message: "监控粒度必须选择", trigger: "change"
        },
    ],
    times: [
        {
            required: true, message: "监控时间必须选择", trigger: "change"
        },
    ]
}
let rules2 = {
    auditUser: [
        {
            required: true, message: "审核负责人必须选择", trigger: "change"
        },
    ],
    explain: [
        {
            required: true, message: "审核提交说明必须填写", trigger: "blur"
        },
    ]
}
let userList = ref([])
...
const openDialog = (record) => {
    ...
    ruleForm0.value.name = recordInfo.value.name || recordInfo.value[`${recordInfo.value.mode}Name`]
